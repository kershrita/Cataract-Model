        ...
        for image_file in image_files:
            ...
            for result in results.boxes.data.tolist():
                ...
                if score > self.threshold:
                    ...
                    # Create subfolder based on class name
                    class_folder = os.path.join(output_path, str(results.names[int(class_id)]))
                    os.makedirs(class_folder, exist_ok=True)

                    # Save the cropped parasite to the subfolder
                    outimg = os.path.join(class_folder, image_file)
                    cv2.imwrite(outimg, cropped_parasite)
                    print(f"{image_file} Cropped and Saved to {class_folder}!")
